...
namespace MSTD_Backend.Services
{
    public class LeetxParser : ParserBase, ILeetxParser
    {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    foreach (var dataRow in tableRows)
                    {
                        ...
                        if (titleNode == null)
                        {
                            Log.Warning($"Could not find title node for torrent {Environment.NewLine} {dataColumns[LeetxTorrentIndexer.Name].OuterHtml}");
                            continue;
                        }
                        ...
                        if (!int.TryParse(dataColumns[LeetxTorrentIndexer.Seeders].InnerText, out var seeders))
                            Log.Warning($"Could not parse seeders {Environment.NewLine}{dataColumns[LeetxTorrentIndexer.Seeders].OuterHtml}");

                        if (!int.TryParse(dataColumns[LeetxTorrentIndexer.Leechers].InnerText, out var leechers))
                            Log.Warning($"Could not parse leechers {Environment.NewLine}{dataColumns[LeetxTorrentIndexer.Leechers].OuterHtml}");
                ...
                {
                    Log.Warning("1337X parse exception", ex);
                    throw;
                }
        ...
        {
            Log.Information("Leetx magnet parsing parsing");
        ...
        public async Task<string> ParsePageForDescriptionHtmlAsync(string pageContents)
        {
            return await Task.Run(() =>
            {
                ...
                {
                    Log.Warning("Could not find description node for 1337X");
                    return string.Empty;
                }
